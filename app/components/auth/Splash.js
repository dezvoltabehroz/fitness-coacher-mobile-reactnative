    ...
    setTimeout(async () => {
      ...
      if (userToken) {
        AuthServices.validateUser(userToken)
          ...
          .catch(async(err) => {await props.authActions.removeUser(props.navigation.replace);console.log(err)})
      } else { await props.authActions.removeUser(props.navigation.replace); }
    }, 2000);
  },[]);
